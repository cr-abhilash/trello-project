...
class Body extends React.Component {
    ...
    async componentDidMount(){
        ...
        try{
          const res= await fetch(`https://api.trello.com/1/members/me/boards?key=${localStorage.getItem("key")}&token=${localStorage.getItem("token")}`)
          if (res.ok) {
    ...
    handleSubmit = async (name1)=>{
        try{
         const res=await fetch(`https://api.trello.com/1/boards?key=${localStorage.getItem("key")}&token=${localStorage.getItem("token")}`,{
            method : 'POST',
            headers: { 
    ...
    return (
        ...
        <div className="boards">
           <FormDialog Stylename="board1 Button" title="Create New Board" submit={this.handleSubmit} label="Enter Board Name"></FormDialog>
